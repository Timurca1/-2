// Функция для индикаторов границ прокрутки
function addScrollIndicators() {
    ...
    function updateScrollIndicators() {
        ...
        tableContainer.classList.remove('scroll-start', 'scroll-middle', 'scroll-end');
...
// Добавляем в обработчик DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
    enhancePriceTable();
    addScrollIndicators();
});

// Функция для улучшения таблицы на мобильных
function enhancePriceTable() {
    const tableContainer = document.querySelector('.table-container');
    const mobileHint = document.querySelector('.mobile-hint');
    
    if (!tableContainer || !mobileHint) return;
    
    // Проверяем, нужно ли показать подсказку
    function checkTableScroll() {
        if (window.innerWidth <= 768) {
            const table = tableContainer.querySelector('table');
            if (table && table.scrollWidth > tableContainer.offsetWidth) {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем класс для стилизации скролла
